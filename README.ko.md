...
**예시**:
```
/planning-plugin:translate social-login              # 전체 동기화
/planning-plugin:translate social-login --section=3  # 섹션 3(기능 요구사항)만 동기화
```
...
### `/planning-plugin:design` (Phase 2 — 추후 지원 예정)

**구문**: `/planning-plugin:design feature-name`

최종화된 명세서를 기반으로 Figma MCP 연동을 통해 Figma 화면 디자인을 생성할 예정입니다. 아직 구현되지 않았습니다.
